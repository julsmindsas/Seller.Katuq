...
import { Pedido, Cliente, EstadoPago, Carrito, EstadoProcesoFiltros, EstadoProceso } from '../../ventas/modelo/pedido';
...
export class POSListOrdersComponent implements OnInit {
  ...
  constructor(
    ...
    private service: ServiciosService, private route: ActivatedRoute, private filterService: FilterService, private ventasService: VentasService, private paymentService: PaymentService, private modalService: NgbModal, private formBuilder: FormBuilder, private pedidoUtilService: PedidosUtilService) {
  ...
  private registerCustomFilters() {
    ...
    this.filterService.register('customDate', (value, filter): boolean => {
      ...
      const dateFormatValue = value.toString().substring(0, 10) + 'T00:00:00'


      return new Date(dateFormatValue).getTime() === filter.getTime();
  ...
  refrescarDatos() {
    ...
    this.ventasService.getOrdersPOSByFilter(filter).subscribe((data: Pedido[]) => {
      console.log(data);
      this.orders = data;
  ...
  pdfOrder(content, order: Pedido) {
    ...
    this.htmlModal = this.paymentService.getHtmlPOSContent(order as POSPedido, this.isFromProduction);
  ...
  produceOrder(order: Pedido) {
    this.producirPedido.emit(order as POSPedido);
  }
  ...
  cambiarEstado(order: POSPedido, estado) {
