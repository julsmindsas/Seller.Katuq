...
@Component({
  selector: 'pos-app-pedido-facturacion',
  templateUrl: 'pos-pedido-facturacion.component.html'
})

export class POSPedidoFacturacionComponent implements OnInit, AfterContentInit {
  ...
  ciudades: string[];
  ciudadesOrigen: any;
  @Output() overridePedido = new EventEmitter<{ pedido: POSPedido, generaFacturaElectronica: boolean }>();
  ...
  ngOnInit() {
    ...
    console.log(this.documentoBusqueda);
    console.log(this.datosFacturacionElectronica);
  }
  ...
  seleccionarDireccionFE(index) {
    ...
    if (this.isEdit) {
      ...
      const dto = {
        pedido: this.pedidoGral,
        generaFacturaElectronica: this.generarFactura
      }
      this.overridePedido.emit(dto);
    }
  }
  ...
  generarFacturaElectronica(event: any) {
    const isChecked = event.target.checked;
    console.log("Checkbox está ", isChecked ? 'activado' : 'desactivado');

    // Guarda el valor del checkbox en la variable this.generarFactura
    this.generarFactura = isChecked;
    const dto = {
      pedido: this.pedidoGral,
      generaFacturaElectronica: this.generarFactura
    }
