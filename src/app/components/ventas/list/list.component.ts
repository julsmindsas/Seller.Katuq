...
export class ListOrdersComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    (this.maestroService.getRol() as any).subscribe({
      next: (roles: any[]) => {
        ...
        this.integrationsService.resolvePrefixes(provider, prefixFilters).subscribe({
          next: (response: any) => {
            ...
            Swal.fire({
              ...
              html: `
                ...
                <hr>
                <p><strong>Seleccione el prefijo de facturación:</strong></p>
              `,
              input: 'select',
              inputOptions,
              inputPlaceholder: 'Seleccione un prefijo',
              icon: 'question',
              showCancelButton: true,
          ...
          error: () => {
            Swal.close();
            this.toastrService.warning('No se pudieron cargar los prefijos. Se usará el prefijo por defecto.', 'Aviso');
            this.mostrarConfirmacionFactura(pedido, providerDisplayName);
          }
        });
      },
  ...
  checkPriceScale(pedido) {
    // spec 010 (T-14): delega al punto único de cálculo (PaymentService).
    // Con el flag `ivaCalcUnificado` ON usa el motor canónico (iva-canonico.ts),
    // eliminando la copia propia de la lógica de precios/volumen del listado.
    return this.paymentService.checkPriceScale(pedido);
  }
  ...
  checkIVAPrice(pedido): {
    ...
    // spec 010 (T-14): delega al punto único de cálculo (PaymentService).
    // Con el flag `ivaCalcUnificado` ON usa el motor canónico (iva-canonico.ts),
    // que resuelve correctamente volumen + IVA override, descuento global y el
    // desglose por tarifa {0,5,8,19} (incluye IVA de envío vía tarifaEnvio, D-058).
    // Reemplaza la copia legacy que aplicaba el IVA override sobre el precio base
    // ignorando la escala de volumen (bug D-046 en el listado).
    return this.paymentService.checkIVAPrice(pedido);
  }
