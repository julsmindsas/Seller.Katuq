...
export class PedidoEntregaComponent implements OnInit, AfterViewInit {
  ...
  guardarDatosEntrega() {
    ...
    if (
      ...
      Swal.fire({
        ...
        text: "Por favor complete al menos los campos de dirección, ciudad y nombres antes de guardar.",
        icon: "warning",
        confirmButtonText: "Ok",
      });
      return;
    ...
    const direccionExistente = this.verificarDireccionDuplicada(datosEntreg);

    if (direccionExistente) {
      Swal.fire({
        title: "Dirección duplicada",
        html: `
                    <p>Ya existe una dirección similar registrada:</p>
                    <div class="text-start mt-3 p-3 bg-light rounded">
                        <strong>Alias:</strong> ${direccionExistente.alias}<br>
                        <strong>Dirección:</strong> ${direccionExistente.direccionEntrega}<br>
                        <strong>Ciudad:</strong> ${direccionExistente.ciudad}
                    </div>
                    <p class="mt-3">¿Desea continuar y crear la dirección de todas formas?</p>
                `,
        icon: "question",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Sí, crear dirección",
        cancelButtonText: "Cancelar",
      }).then((result) => {
        if (result.isConfirmed) {
          this.procederConGuardado(datosEntreg);
        }
      });
    } else {
      this.procederConGuardado(datosEntreg);
  ...
  /**
   * Verifica si ya existe una dirección duplicada
   * @param nuevaDireccion Los datos de la nueva dirección a verificar
   * @returns La dirección existente si hay duplicado, null si no existe
   */
  private verificarDireccionDuplicada(nuevaDireccion: any): any {
    if (!this.datosEntregas || this.datosEntregas.length === 0) {
      return null;
    }

    return this.datosEntregas.find((direccionExistente) => {
      // Normalizar textos para comparación (eliminar espacios extra, convertir a minúsculas)
      const direccionExistenteNorm = direccionExistente.direccionEntrega
        ?.toLowerCase()
        .trim();
      const direccionNuevaNorm = nuevaDireccion.direccionEntrega
        ?.toLowerCase()
        .trim();
      const ciudadExistenteNorm = direccionExistente.ciudad
        ?.toLowerCase()
        .trim();
      const ciudadNuevaNorm = nuevaDireccion.ciudad?.toLowerCase().trim();

      // Verificar si la dirección y ciudad son exactamente iguales
      const direccionIgual = direccionExistenteNorm === direccionNuevaNorm;
      const ciudadIgual = ciudadExistenteNorm === ciudadNuevaNorm;

      // También verificar si tienen el mismo código postal (si ambos lo tienen)
      const codigoPostalIgual =
        direccionExistente.codigoPV && nuevaDireccion.codigoPV
          ? direccionExistente.codigoPV === nuevaDireccion.codigoPV
          : true; // Si alguno no tiene código postal, no considerarlo para la comparación

      return direccionIgual && ciudadIgual && codigoPostalIgual;
    });
  }

  /**
   * Procede con el guardado de la dirección después de las validaciones
  ...
  /**
   * Limpia los campos del formulario de entrega
   */
  private limpiarFormularioEntrega() {
    this.alias_entrega = "";
    this.nombres_entrega = "";
