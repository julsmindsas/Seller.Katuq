    ...
    guardarDatosEntrega() {
        ...
        this.service.getClientByDocument(data).subscribe((res: any) => {
            ...
            this.service.editClient(this.formulario.value).subscribe((r) => {
                ...
                this.service.getClientByDocument({documento: this.documentoBusqueda}).subscribe((clientRes: any) => {
                    if (clientRes && clientRes.datosEntrega && Array.isArray(clientRes.datosEntrega)) {
                    ...
                    if (this.datosEntregas.length == 0) {
                        ...
                        Swal.fire({
                            title: "Advertencia!",
                            text: "Se guardó con éxito, pero recuerda que debes tener una dirección de entrega para la ciudad seleccionada (" + (this.pedidoGral?.envio?.ciudad || 'seleccionada') + ") y posiblemente la que se creó no se vea reflejada en la lista de direcciones de entrega.",
                            icon: "warning",
                            confirmButtonText: "Ok",
                        });
                    }
            ...
            (resultado) => {
                ...
                if (typeof resultado === 'object') {
                    ...
                    if (resultado.coordenadas) {
                        ...
                        const coords = resultado.coordenadas.split(',').map((coord: string) => coord.trim());
