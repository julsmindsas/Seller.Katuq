...
- [ ] 1.1 `kapsoService.sendInteractive({to, cuerpo, opciones, company})`:
...
- [ ] 1.2 Camino de salida: `enviarTexto` acepta `opciones` y delega en
...
- [ ] 2.1 Webhook: el payload del bot lleva `opcionId` cuando el mensaje es
      `interactive` (button_reply / list_reply). Tests.
- [ ] 2.2 Despachador: `interactive` deja de caer en el aviso de medios y se
...
- [ ] 3.1 Herramienta `ofrecer_opciones(pregunta, opciones)` con
      etiqueta/valor/descripción; anota en el estado del turno; el endpoint
      las devuelve. Tests puros.
- [ ] 3.2 Instrucciones: cuándo conviene ofrecer opciones (conjunto cerrado y
...
- [ ] 4.1 Suites backend + ADK en verde.
