...
- [x] 1.1 Lector del maestro `formaEntrega` por comercio (solo lectura, caché
      5 min como `formasDePago`; clasifica domicilio/recoge por nombre).
      Tests.
- [x] 1.2 Sesión del bot: llaves `entrega` (forma, fecha, esRecoge), `pago`,
...
- [x] 1.3 Despachador: manda al agente las formas de entrega reales + lo ya
...
- [x] 1.4 Gates de cierre: sin forma + fecha válidas no hay cierre (motivo
      claro al traspasar); domicilio sigue exigiendo dirección. Tests.
- [x] 1.5 Cierre: ítems con `configuracion` canónica de venta asistida
...
- [x] 2.1 Herramientas locales `configurar_entrega(forma, fecha)`,
...
- [x] 2.2 Instrucciones: preguntar entrega + fecha juntas cuando se pueda,
      ...
      instrucciones.
- [x] 2.3 Endpoint: recibe formasEntrega + config ya capturada, devuelve
      entrega/pago/dedicatoria/facturacion del turno. Tests.
- [x] 2.4 Instrucciones de VENTA: sugerir un complemento pertinente (solo de
...
- [x] 3.1 Backend: descarga de media del proveedor (GET media por id con las
...
- [x] 3.2 Despachador: tipo imagen deja de ser aviso cortés y va al agente
...
- [x] 3.3 ADK: endpoint y pipeline aceptan imagen (base64 + mime) y arman el
      turno multimodal; instrucciones para describir-buscar-ofrecer y para
      pedir texto cuando la imagen no se entienda. Tests.
- [x] 3.4 Fallo limpio: imagen pesada/formato raro → respuesta amable pidiendo
      texto; error de descarga → aviso cortés de siempre. Nunca truena.

## 4. Verificación

- [x] 4.1 Suites backend + ADK en verde; contract test del write-set intacto.
