...
- **Pendientes (fases siguientes):** (1) sourcemaps al build de prod (requiere `SENTRY_AUTH_TOKEN` + `sentry-cli` en `npm run release` — sin esto los stacks de prod salen minificados); (2) proyecto backend `@sentry/node` en el Express + quitar winston muerto; (3) alertas a Slack; (4) revisar ruido de extensiones de navegador (`denyUrls`) tras primeros días de datos.
