...
| **016** | **facturacion-dian-directa** | **discovery — manuales de onboarding listos** | Daniel | Facturación electrónica DIRECTA con la DIAN (modo "software propio" / desarrollo tecnológico adquirido por empresa, Res. 000165/2023 art. 28) mientras se evalúa proveedor marca blanca (candidato: Factus). Fase 0 entregada: manuales de onboarding en `docs/manuales/facturacion-dian/` (guía cliente 5 pasos + runbook interno Katuq F1-F7). Pendiente: spec del motor (UBL 2.1 v1.9 + XAdES-EPES + SOAP DIAN como `dianProvider` del `AccountingManager`). Rama `feature/facturacion-dian-directa`. Ver D-079. |
